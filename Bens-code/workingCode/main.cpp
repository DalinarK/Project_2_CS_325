...
void getinput(std::ifstream &inStream, std::vector<std::vector<int> > &returnCoinLists, std::vector<int> &returnTotalValue);
	...
	*/
    ...
    for( unsigned int i = 0; i < coinsetinput.size(); i++ )
    {
		if(!coinsetinput.at(i).empty()){
            ...
            Changedp::dpChange(coinsetinput.at(i), changevalueV.at(i), result);	
